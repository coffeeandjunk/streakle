...
if (Meteor.isClient) {
    ...
    Session.setDefault('profilePostsLimit', ITEMS_INCREMENT);
}
...
ProfilePostsController = RouteController.extend({
    ...
    increment: 5,
    profilePostsLimit: function() {
        return Session.get('profilePostsLimit');
    },
    findOptions: function() {
        return {
            sort: {
    ...
    data: function() {
        var hasMore = this.profilePosts().count() === this.profilePostsLimit();
        var nextPath = this.route.path({
            profilePostsLimit: this.profilePostsLimit() + ITEMS_INCREMENT
        });
        return {
